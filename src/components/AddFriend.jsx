...
const AddFriend = ({ profile }) => {
  ...
  const rejectedFriend = async () => {
    try { 
      ...
      if (!isConfirmed) return;
      setFriendStatus("rejected"); // ✅ Cập nhật state
    } catch (error) {
  ...
  const statusConfig = {
    ...
    pending: { icon: XMarkIcon, text: "Từ chối", color: "text-red-500" },
    accepted: { icon: UserMinusIcon, text: "Unfriend", color: "text-gray-500" },
    noFriend: { icon: UserPlusIcon, text: "Add Friend", color: "text-green-500" },
    blocked: { icon: UserPlusIcon, text: "Bị block", color: "text-gray-500" },
    rejected: { icon: UserPlusIcon, text: "Gửi lại yêu cầu", color: "text-gray-500" },
  };
    ...
    <>
      <button
        ...
        className={`bg-gray-50 px-2 py-2 rounded-md flex items-center transition-transform duration-200 ${
          loading ? "opacity-50 cursor-not-allowed" : "hover:bg-green-50 hover:scale-110"
        }`}
      >
        {loading ? (
